"use client";
import type { AnalyzeSources, Route, Stop } from "@/lib/types";
...
export default function ItinerarySheet({ route, saved, onClose, onShow, onSave, onShare }: {
  route: Route; saved: boolean; onClose: () => void; onShow: (s: Stop) => void; onSave: () => void; onShare: () => void;
}) {
  ...
  return (
    <Sheet label={route.name} onClose={onClose}>
      ...
      <span className="eyebrow">{route.kind === "plan" ? "TU VIAJE A MEDIDA" : route.kind === "demo" ? "RUTA DE EJEMPLO" : "RUTA DE TU VÍDEO"}</span>
      <h2>{route.name}</h2>
